...
class CarModel():
    ...
    def __init__(self):
        ...
        self.bodyPosData = []
        self.bodyAccelData = []
        self.spring1ForceData = []  # Force data for spring 1
        self.spring2ForceData = []  # Force data for spring 2
        self.dashpotForceData = []  # Force data for the dashpot
...
class CarController():
    ...
    def ode_system(self, t ,X):
        """
            Defines the system of differential equations for the quarter car model.
            Calculates forces in the springs and the dashpot based on their deformations and stores them.
            """
        x1, x1dot, x2, x2dot = X
        ...
        # Spring and dashpot forces calculations
        spring1_force = self.model.k1 * (x2 - x1)
        spring2_force = self.model.k2 * (y - x2)
        dashpot_force = self.model.c1 * (x2dot - x1dot)

        # Append forces to model data arrays
        self.model.spring1ForceData.append(spring1_force)
        self.model.spring2ForceData.append(spring2_force)
        ...
        x1ddot = (1 / self.model.m1) * (-spring1_force - dashpot_force)
        x2ddot = (1 / self.model.m2) * (spring1_force + spring2_force + dashpot_force)
