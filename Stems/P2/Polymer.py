...
def simulate_polymers(N, count):
    ...
    :param N: Target degree of polymerization.
    :param count: Number of macromolecules to simulate.
    :return: Prints the calculated metrics to the console.
    """
    degrees_of_polymerization = np.random.normal(N, 0.1 * N, count)
    polymers = [macroMolecule(int(degree)) for degree in degrees_of_polymerization]

    centers_of_mass = [poly.centerOfMass.getTup() for poly in polymers]
    end_to_end_distances = [poly.endToEndDistance for poly in polymers]
    radii_of_gyration = [poly.radiusOfGyration for poly in polymers]
