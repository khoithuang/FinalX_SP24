...
#endregion
# Assuming that the macroMolecule class and Position class have been defined as per the previous code snippet

def simulate_polymer_molecules(N_mean, num_molecules):
    N_std_dev = 0.1 * N_mean
    polymer_molecules = []

    for _ in range(num_molecules):
        degree_of_polymerization = int(np.random.normal(N_mean, N_std_dev))
        polymer = macroMolecule(degreeOfPolymerization=degree_of_polymerization)
        polymer.freelyJointedChainModel()
        polymer_molecules.append(polymer)

    # Compute statistics
    centers_of_mass = [m.centerOfMass.getTup() for m in polymer_molecules]
    end_to_end_distances = [m.endToEndDistance for m in polymer_molecules]
    radii_of_gyration = [m.radiusOfGyration for m in polymer_molecules]
    molecular_weights = [m.MW for m in polymer_molecules]

    avg_center_of_mass = np.mean(centers_of_mass, axis=0)
    avg_end_to_end_distance = mean(end_to_end_distances)
    std_end_to_end_distance = stdev(end_to_end_distances)
    avg_radius_of_gyration = mean(radii_of_gyration)
    std_radius_of_gyration = stdev(radii_of_gyration)
    pdi = stdev(molecular_weights) / mean(molecular_weights)

    return avg_center_of_mass, avg_end_to_end_distance, std_end_to_end_distance, avg_radius_of_gyration, std_radius_of_gyration, pdi

def main():
    N_default = 1000
    num_molecules_default = 50

    N = int(input(f"degree of polymerization ({N_default})?: ") or N_default)
    num_molecules = int(input(f"How many molecules ({num_molecules_default})?: ") or num_molecules_default)

    results = simulate_polymer_molecules(N, num_molecules)

    print(f"Metrics for {num_molecules} molecules of degree of polymerization = {N}")
    print(f"Avg. Center of Mass (nm) = {results[0][0]:.3f}, {results[0][1]:.3f}, {results[0][2]:.3f}")
    print(f"End-to-end distance (μm):\n\tAverage = {results[1]:.3f}\n\tStd. Dev. = {results[2]:.3f}")
    print(f"Radius of gyration (μm):\n\tAverage = {results[3]:.3f}\n\tStd. Dev. = {results[4]:.3f}")
    print(f"PDI = {results[5]:.2f}")
